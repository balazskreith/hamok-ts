...
import { HamokChannelSnapshot } from '../HamokSnapshot';
import { v4 as uuid } from 'uuid';
import { HamokChannelSubscriptions } from './HamokChannelSubscriptions';
import { HamokChannelPendingRequest } from './HamokChannelPendingRequest';

export const logger = createLogger('HamokChannel');
...
type RequestListenerParameters<L> = L extends HamokChannelRequestHandler<any, any> ? Parameters<L> : never
// eslint-disable-next-line @typescript-eslint/no-explicit-any
// type NotificationListenerParameters<L extends (...args: any) => any> = L extends HamokChannelRequestHandler<any, any> ? never : Parameters<L>

export type HamokChannelNotificationHandler<T = unknown> = (data: T) => void;
export type HamokChannelRequestHandler<Input = unknown, Output = unknown> = (data: Input) => Promise<Output>;
export type HamokChannelHandler<T = unknown, O = unknown> = HamokChannelNotificationHandler<T> | HamokChannelRequestHandler<T, O>;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export type HamokChannelEventMap = Record<string, HamokChannelHandler<any, any>>;

export type HamokChannelResponseMessage<T = unknown> = {
	type: 'response';
	requestId: string;
	payload?: T;
	error?: string;
...
export type HamokChannelRequestMessage<T = unknown> = {
	type: 'request';
	requestId: string;
	event: string;
	data: T;
...
export type HamokChannelNotificationMessage<T = unknown> = {
	type: 'notification';
	event: string;
	data: T;
}

export type HamokChannelMessage = HamokChannelRequestMessage | HamokChannelNotificationMessage | HamokChannelResponseMessage;

export type HamokChannelStats = {
	numberOfSubscriptions: number;
	numberOfReceivedEventInvocations: number;
	numberOfSentEventInvocations: number;
}

export class HamokChannel<T extends HamokChannelEventMap, M extends Record<string, unknown> = Record<string, unknown>> {
	// private readonly _subscriptions = new Map<keyof T, Set<string>>();
	public readonly subscriptions = new HamokChannelSubscriptions<T, M>();
	private readonly _emitter = new EventEmitter();
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	private readonly _pendingRequests = new Map<string, HamokChannelPendingRequest<any>>();

	private _initializing?: Promise<this>;
	private _closed = false;

	public stats: HamokChannelStats = {
	...
	public constructor(
		...
		this.connection
			...
			.on('UpdateEntriesNotification', async (notification) => {
				// this is for the events to emit
				for (const [ event, serializedMessage ] of notification.updatedEntries) {
					try {
						const message = JSON.parse(serializedMessage) as HamokChannelMessage;

						if (message.type === 'notification') {
							if (Array.isArray(message.data)) this._emitter.emit(message.event, ...message.data);
							else this._emitter.emit(message.event, message.data);
						
							++this.stats.numberOfReceivedEventInvocations;

							continue;
						}
						const responses: HamokChannelResponseMessage[] = [];
						...
						for (const listener of listeners as HamokChannelRequestHandler[]) {
							try {
								const payload = await listener(message.data);
						...
						if (0 < responses.length) {
							this.connection.notifyEntryUpdated(
								message.event, 
								'oldValue', 
			...
			.on('EntryUpdatedNotification', (notification) => {
				if (!notification.sourceEndpointId) return logger.warn('%s Received an unexpected response for %s. %o', this.connection.localPeerId, this.id, notification);
				try {
					const event = notification.key;
					const responses = JSON.parse(notification.newValue) as HamokChannelResponseMessage[];
					const responsesByRequests = Collections.groupArrayBy(responses, (r) => r.requestId);

					for (const [ requestId, requestResponses ] of responsesByRequests.entries()) {
						const pendingRequest = this._pendingRequests.get(requestId);

						if (!pendingRequest) return logger.warn('%s Received an unexpected response for %s. %o', this.connection.localPeerId, event, notification);
						
						const error = requestResponses.find((r) => r.error !== undefined);

						if (error) {
							return pendingRequest.reject(notification.sourceEndpointId, error.error as string);
						}

						pendingRequest.accept(
							notification.sourceEndpointId, 
							...(requestResponses.map((r) => r.payload))
						);
					}
				} catch (err) {
					logger.error('Error while decoding the payload for %s, %s, %o', this.id, notification.key, `${err}`);
				}
			})
			...
			.on('remote-snapshot', (serializedSnapshot, done) => {
				try {
					const snapshot = JSON.parse(serializedSnapshot) as HamokChannelSnapshot;
	...
	public async subscribe<K extends keyof T>(event: K, listener: T[K], metaData: M | null = null): Promise<void> {
	...
	public async unsubscribe<K extends keyof T>(event: K, listener: T[K]): Promise<void> {
	...
	public async request<K extends keyof T, R extends Awaited<ReturnType<T[K]>>>(event: K, ...args: RequestListenerParameters<T[K]>): Promise<R[]> {
		...
		const pendingRequest = new HamokChannelPendingRequest<R>(
			requestId,
			0,
			remotePeerIds,
			10000,
		);
		const entry = [ event as string, pendingRequest.getMessage(event as string, ...args) ] as [string, string];
	...
	public notify<K extends keyof T>(event: K, ...args: Parameters<T[K]>): boolean {
		if (this._closed) throw new Error('Cannot publish on a closed emitter');
		...
		const message: HamokChannelNotificationMessage = {
			type: 'notification',
			event: event as string,
			data: args,
		};
		const entry = [ event as string, JSON.stringify(message) ] as [string, string];
	...
	public export(): HamokChannelSnapshot {
		if (this._closed) throw new Error('Cannot export a closed emitter');

		const subscriptions: HamokChannelSnapshot['subscriptions'] = [];
        
		for (const [ event, peerMap ] of this.subscriptions.entries()) {
			const subscribers: HamokChannelSnapshot['subscriptions'][number]['subscribers'] = [];
	...
	public import(snapshot: HamokChannelSnapshot): void {
	...
	private _import(snapshot: HamokChannelSnapshot): void {
